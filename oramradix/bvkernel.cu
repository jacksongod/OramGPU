...
typedef MetaTree<TREESIZE,BLOCKPERBUCKET> OramB; 
...
__global__ void oramshare(uint16_t* position_table, uint32_t* access_script,uint16_t* checktable, OramB* oramtree, TDBlock<BLOCKSIZE>* checktable2,curandState *randstate,OramD* datatree){
	int tid =  threadIdx.x;
        ...
        __shared__ OramB metatree;              //4K*sizeof(OramB) = 16KB
       // __shared__ OramB treepath[LEAFNUMLOG+1];          //12*sizeof(OramB) = 48B
        ...
        __shared__ uint32_t stash_start;            //256B
  ...
  //      __syncthreads();
		...
		//if (!blockinstash) {
         //if (tid< 384 && tid%32==0){  //copy entire path to local registers (12 levels, 24 blocks)
	   if (tid<24){
               //int stid = tid /32;
               //int f = tid*2;
               //int s = f+1;
           ...
           uint16_t id = metatree.idlist[tid%2].bucket[treeindex];
           //uint16_t id1 = metatree.idlist[1].bucket[treeindex];
		   metatree.idlist[tid%2].bucket[treeindex] = 0;
		//   metatree.idlist[1].bucket[treeindex] = 0;

		
              //stashaccessloc[stid] = positionindex;
              stash[(stash_empty_ptr+tid)%STASHSIZE] = id ;
              //stash[(stash_empty_ptr+s)%STASHSIZE] = id1 ;
...
//	 cudaThreadSetCacheConfig(cudaFuncCachePreferL1);
   ...
   setup_kernel<<<CUDABLOCKNUM,CUDATHREADNUM>>>(devStates);
    ...
    int startpoint =  (1<<LEAFNUMLOG) -1 ; 
		...
		oramtree->initbucketzero(i);

    	}
    for (int i = startpoint; i< (TREESIZE-1); i++){
            int temp = i- startpoint;
        doramtree[i].init(); 
        oramtree->initbucket(i,temp*BLOCKPERBUCKET); 
   ...
   // cudaMalloc((void**)&cuorampath,sizeof(uint32_t) *( 1<<LEAFNUMLOG));
    cudaMalloc((void**)&cuoramtree,sizeof(OramB) );
   ...
   // cudaMemcpy(cuorampath, orampath, (1<<LEAFNUMLOG) * sizeof(uint32_t),cudaMemcpyHostToDevice);
    cudaMemcpy(cuoramtree, oramtree,  sizeof(OramB),cudaMemcpyHostToDevice);
   ...
   // CUDA_CALL(cudaMemcpy(doramtree,cudoramtree,(TREESIZE)*sizeof(OramD),cudaMemcpyDeviceToHost));
    ...
    for (int i =0 ; i< ACCESSNUM ; i++){
       ...
       for(int j = 11 ; j >= 0; j--) {
        ...
        if (check_table[i*24+j*2] !=  oramtree->idlist[0].bucket[bucketindex]){
            pass = false; 
            printf("fail 0 id: 0x%x 0x%x real id 0x%X,  0x%x\n" ,bucketindex,oramtree->idlist[0].bucket[bucketindex],check_table2[i*24+j*2], check_table[i*24+j*2] );
        }
        else
        { 
            printf("pass 0 id: 0x%x 0x%x real id 0x%x, 0x%x\n" ,bucketindex,oramtree->idlist[0].bucket[bucketindex],check_table2[i*24+j*2] ,check_table[i*24+j*2] );
        }
        if (check_table[i*24+j*2+1] !=  oramtree->idlist[1].bucket[bucketindex]){
            pass = false;
            printf("fail 1 id: 0x%x 0x%x real id 0x%x 0x%x\n" ,bucketindex, oramtree->idlist[1].bucket[bucketindex], check_table2[i*24+j*2+1],check_table[i*24+j*2+1] );
           
        }
        else
        { 
            printf("pass 0 id: 0x%x 0x%x real id 0x%x 0x%x\n" ,bucketindex,oramtree->idlist[1].bucket[bucketindex],check_table2[i*24+j*2+1] ,check_table[i*24+j*2] );
        }
